...
Function EncodeBase64_:String( data:UByte Ptr, length:Int, mode:EncodeBase64_Modes_=EncodeBase64_Modes_.Standard, pad:Bool=True, lnlength:UInt=Null, delimiter:String="~r~n" )
	
	Print "encode: "+CharToString( data )
	
	Local buf:=New Stack<UByte>
	Local tmp:=New UByte[3]
	Local i:=0, j:=0, lineCount := 0
	Local charDelim:=StringToCharArray(delimiter)
	Local delimLen:=charDelim.Length

	' Set mode-specific configurations
	Base64Setup(Varptr(mode), Varptr(pad), Varptr(lnlength), Null, Varptr(charDelim[0]), Varptr(delimiter), Null, Null)
	...
	While i < length
		...
		If j = 3
			...
			' Insert delimiters for RFC1421 and RFC2045 modes
			If lnlength And lineCount >= lnlength
	...
	' Inject fake checksum for RFC1421 and RFC9580 modes
...
Function DecodeBase64_:DataBuffer( str:String, mode:EncodeBase64_Modes_=EncodeBase64_Modes_.Standard, pad:Bool=True, lnlength:UInt=Null, delimiter:String="~r~n" )
	
	Print "str to decode: "+str.Replace("~n","")
...
#end
Function EncodeBase64_old:String( 	data:UByte Ptr,length:Int,
...
| URL-safe  | Optional                      |
#end
Function DecodeBase64_old:DataBuffer( 	str:String,
									mode:EncodeBase64_Modes_=EncodeBase64_Modes_.Standard,
									pad:Bool=True,
									lnlength:UInt=Null,
...
'		If c=CHAR_EQUALS Or c<0 Or c>=decode.Length Or decode[c]=-1 Exit
		...
		#Rem
		Improved removal of non-alphabet characters for RFC1421 and RFC2045 modes.
		Ensured fake checksum bytes are removed correctly for RFC1421 and RFC9580.
		#End
'		If c<0 Or c>=decode.Length Or decode[c]=-1
'			' Skip non-alphabet characters (e.g., line breaks, spaces) for RFC modes allowing them
'			If nonEncodingChar Or c=CHAR_EQUALS
'				i+=1
'				Continue
'			Else
'				' Exit on invalid Base64 characters if nonEncodingChar is False
'				Exit
'			End
'		End
...
'		If c=CHAR_EQUALS Or c<0 Or c>=decode.Length Or decode[c]=-1 Exit
		...
		If c<0 Or c>=decode.Length Or decode[c]=-1
			' Skip non-alphabet characters (e.g., line breaks, spaces) for modes that allow them
			If nonEncodingChar
				i+=1
'				Exit
				Continue
			End
			' Handle padding character '=' for proper termination
			If c=CHAR_EQUALS
				Exit
			End
			' Exit on invalid Base64 characters if nonEncodingChar is False
